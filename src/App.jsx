          ...
          <div className="join-form">
            ...
            <div className="room-grid">
              {ROOMS.map((r) => (
                <button
                  key={r}
                  className={`room-chip ${room === r ? "active" : ""}`}
                  onClick={() => setRoom(r)}
                >
                  # {r}
                </button>
    ...
    <div className="chat-layout">
      ...
      <aside className="sidebar">
          ...
          <div className="user-list">
            {users.map((u) => (
              <div key={u.id} className="user-item">
                <div className="avatar-sm" style={{ background: u.color }}>
                  {getInitials(u.username)}
                </div>
                <span className={u.id === myId.current ? "me" : ""}>
                  {u.username}
                  {u.id === myId.current ? " (you)" : ""}
                </span>
              </div>
            ))}
          </div>
        ...
        <div className="sidebar-footer">
          <div className="avatar-sm" style={{ background: "#4ECDC4" }}>
          ...
          {messages.map((msg, i) => {
            ...
            return (
              <div key={msg.id} className={`msg ${isMe ? "mine" : "theirs"} ${grouped ? "grouped" : ""}`}>
                {!grouped && (
                  <div className="avatar-sm" style={{ background: msg.color }}>
